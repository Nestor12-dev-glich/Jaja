--[[
    NINO HUB + PRIME GRAB + NRHUB FEATURES
    - Auto Grab (Prime Hub) con barra blanca, ping/fps
    - Anti Bat (Fling Defense) + Insta Reset + Medusa Auto Reset
    - Bat Aimbot (NRHub) con highlight y predicción
...
-- ============================================================
-- NRHUB FEATURES: ANTI BAT, INSTA RESET, MEDUSA AUTO RESET
-- ============================================================

-- ANTI BAT (Fling Defense)
local AntiBat = {
    active = false,
    conn = nil,
    FORCE = 1000,
...
-- INSTA RESET (con remote hook)
local RESET_GUID = "f888ee6e-c86d-46e1-93d7-0639d6635d42"
local resetRemote = nil
local resetCooldown = false

pcall(function()
    local orig = hookfunction(Instance.new("RemoteEvent").FireServer, newcclosure(function(self, ...)
...
-- ============================================================
-- BAT AIMBOT (NRHub style con highlight y predicción)
-- ============================================================
...
local S = {
    ...
    batAimbotEnabled = false, batAimbotSetVisual = nil,
    ...
    instaResetEnabled = false, setInstaResetVisual = nil,
    KB = {
        ...
        InstaReset = {kb = Enum.KeyCode.K, gp = nil},
    },
    AP = {
...
local function updateAutoPlayUI() end

-- AUTO LEFT / RIGHT (con AutoPlay)
...
local function saveConfig()
    pcall(function()
        ...
        local cfg = {
            ...
            autoGrabEnabled = AutoGrab.AUTO_STEAL_ENABLED, autoGrabRange = AutoGrab.PRIME_RANGE, autoGrabDuration = AutoGrab.HOLD_MAX,
            antiBatEnabled = AntiBat.active, medusaAutoResetEnabled = medusaAutoResetEnabled,
            antiBatKey = ks(S.KB.AntiBat), instaResetKey = ks(S.KB.InstaReset),
        }
...
local function buildMoveTab(pages)
    local batToggleVisual, _ = mkToggle("Move", pages, "Bat Aimbot (NR)", S.KB.AutoBat.kb, false, function(on)
        if on then if AutoPlay.Enabled then setAutoPlay(AutoPlay.Side, false) end; startBatAimbot() else stopBatAimbot() end
    ...
    local setALVis, _ = mkToggle("Move", pages, "Auto Left", S.KB.AutoLeft.kb, false, function(on)
        if on then if AutoPlay.Enabled and AutoPlay.Side == "right" then setAutoPlay("right", false) end; if batAimbotEnabled then stopBatAimbot(); batToggleVisual(false) end; setAutoPlay("left", true)
        else if AutoPlay.Enabled and AutoPlay.Side == "left" then setAutoPlay("left", false) end end
    ...
    local setARVis, _ = mkToggle("Move", pages, "Auto Right", S.KB.AutoRight.kb, false, function(on)
        if on then if AutoPlay.Enabled and AutoPlay.Side == "left" then setAutoPlay("left", false) end; if batAimbotEnabled then stopBatAimbot(); batToggleVisual(false) end; setAutoPlay("right", true)
        else if AutoPlay.Enabled and AutoPlay.Side == "right" then setAutoPlay("right", false) end end
        S.restartMovement(); updateFloatingButtons(); saveConfig()
    ...
    actionRow("Move", "Insta Reset", S.KB.InstaReset)
end
...
local function buildGui()
    ...
    UIS.InputBegan:Connect(function(input, gpe)
        ...
        elseif match(S.KB.AutoLeft) then
            ...
            else if AutoPlay.Enabled and AutoPlay.Side == "right" then setAutoPlay("right", false) end; if batAimbotEnabled then stopBatAimbot(); if S.batAimbotSetVisual then S.batAimbotSetVisual(false) end end; setAutoPlay("left", true) end
            S.restartMovement(); updateFloatingButtons(); saveConfig()
        elseif match(S.KB.AutoRight) then
            if AutoPlay.Enabled and AutoPlay.Side == "right" then setAutoPlay("right", false)
            else if AutoPlay.Enabled and AutoPlay.Side == "left" then setAutoPlay("left", false) end; if batAimbotEnabled then stopBatAimbot(); if S.batAimbotSetVisual then S.batAimbotSetVisual(false) end end; setAutoPlay("right", true) end
            S.restartMovement(); updateFloatingButtons(); saveConfig()
        elseif match(S.KB.AutoBat) then
            ...
            if S.batAimbotSetVisual then S.batAimbotSetVisual(newState) end
            updateFloatingButtons(); saveConfig()
...
local function createFloatingButtonPanel()
    ...
    btnBAT.MouseButton1Click:Connect(function()
        ...
        if newState then startBatAimbot() else stopBatAimbot() end
        if S.batAimbotSetVisual then S.batAimbotSetVisual(newState) end
        setButtonActive(btnBAT, bsBAT, l1BAT, l2BAT, newState)
        updateFloatingButtons(); saveConfig()
    end)
    btnAL.MouseButton1Click:Connect(function()
        if AutoPlay.Enabled and AutoPlay.Side == "left" then
            ...
            if batAimbotEnabled then stopBatAimbot(); setButtonActive(btnBAT, bsBAT, l1BAT, l2BAT, false); if S.batAimbotSetVisual then S.batAimbotSetVisual(false) end end
...
local function loadConfig()
    ...
    if cfg.batAimbot then startBatAimbot(); if S.batAimbotSetVisual then S.batAimbotSetVisual(true) end end
    ...
    if cfg.autoGrabEnabled ~= nil then AutoGrab.AUTO_STEAL_ENABLED = cfg.autoGrabEnabled; if S.setInstaGrab then S.setInstaGrab(AutoGrab.AUTO_STEAL_ENABLED) end; if AutoGrab.AUTO_STEAL_ENABLED then startAutoSteal() else stopAutoSteal() end end
