--[[
    NINO HUB - FULL INTEGRADO
    - Auto Play (izquierda/derecha) desde Atr Auto Play
    - Auto Grab/Steal mejorado (barra blanca, ping/fps)
    - Todas las funciones originales de NINO HUB
]]
...
-- ============================================================
-- AUTO PLAY (Atr Auto Play) - MOVIMIENTO AUTOMÁTICO
-- ============================================================
local AutoPlay = {
    Enabled = false,
...
-- AUTO PLAY UI TOGGLE
-- ============================================================
local autoPlayLeftBtn = nil
local autoPlayRightBtn = nil
local autoPlayGoingSpeedBox = nil
local autoPlayStealSpeedBox = nil

local function updateAutoPlayUI()
    if autoPlayLeftBtn then
        autoPlayLeftBtn.BackgroundColor3 = (AutoPlay.Enabled and AutoPlay.Side == "left") and Color3.fromRGB(80, 80, 100) or Color3.fromRGB(45, 45, 55)
        autoPlayLeftBtn.Text = (AutoPlay.Enabled and AutoPlay.Side == "left") and "AUTO LEFT: ON" or "AUTO LEFT: OFF"
    end
    if autoPlayRightBtn then
        autoPlayRightBtn.BackgroundColor3 = (AutoPlay.Enabled and AutoPlay.Side == "right") and Color3.fromRGB(80, 80, 100) or Color3.fromRGB(45, 45, 55)
        autoPlayRightBtn.Text = (AutoPlay.Enabled and AutoPlay.Side == "right") and "AUTO RIGHT: ON" or "AUTO RIGHT: OFF"
    end
end
...
-- AUTO STEAL (reemplazado por AutoGrab)
local function startAutoStealOld() end
local function stopAutoStealOld() end
...
local function buildGui_createScrollingPages(rightPanel)
    local pages = {}
    for _, n in ipairs({"Speed", "Main", "Move", "Config", "Auto"}) do
...
local rowCounts = {Speed = 0, Main = 0, Move = 0, Config = 0, Auto = 0}
...
local function buildConfigTab(pages)
    ...
    S.radInput = mkInput("Config", pages, "Steal Radius", AutoGrab.PRIME_RANGE, function(v)
...
-- AUTO PLAY TAB
-- ============================================================
local function buildAutoPlayTab(pages)
    local C_WHITE = Color3.fromRGB(255,255,255)
    local C_ON = Color3.fromRGB(128,0,128)
    local C_OFF = Color3.fromRGB(60,60,60)
    ...
    -- Botón Auto Left
    do
        local card = mkCard("Auto", pages, 38)
        local lbl = Instance.new("TextLabel", card)
        lbl.Size = UDim2.new(0,100,1,0); lbl.Position = UDim2.new(0,12,0,0)
        lbl.BackgroundTransparency = 1; lbl.Text = "Auto Left"; lbl.TextColor3 = C_WHITE
        lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 11
        
        autoPlayLeftBtn = Instance.new("TextButton", card)
        autoPlayLeftBtn.Size = UDim2.new(0,80,0,28)
        autoPlayLeftBtn.Position = UDim2.new(1,-90,0.5,-14)
        autoPlayLeftBtn.BackgroundColor3 = Color3.fromRGB(45,45,55)
        autoPlayLeftBtn.Text = "AUTO LEFT: OFF"
        autoPlayLeftBtn.TextColor3 = C_WHITE
        autoPlayLeftBtn.Font = Enum.Font.GothamBold
        autoPlayLeftBtn.TextSize = 10
        Instance.new("UICorner", autoPlayLeftBtn).CornerRadius = UDim.new(0,6)
    ...
    do
        ...
        local lbl = Instance.new("TextLabel", card)
        lbl.Size = UDim2.new(0,100,1,0); lbl.Position = UDim2.new(0,12,0,0)
        lbl.BackgroundTransparency = 1; lbl.Text = "Auto Right"; lbl.TextColor3 = C_WHITE
        lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 11
        
        autoPlayRightBtn = Instance.new("TextButton", card)
        autoPlayRightBtn.Size = UDim2.new(0,80,0,28)
        autoPlayRightBtn.Position = UDim2.new(1,-90,0.5,-14)
        autoPlayRightBtn.BackgroundColor3 = Color3.fromRGB(45,45,55)
        autoPlayRightBtn.Text = "AUTO RIGHT: OFF"
        autoPlayRightBtn.TextColor3 = C_WHITE
        autoPlayRightBtn.Font = Enum.Font.GothamBold
        autoPlayRightBtn.TextSize = 10
        Instance.new("UICorner", autoPlayRightBtn).CornerRadius = UDim.new(0,6)
    ...
    -- Going Speed
    autoPlayGoingSpeedBox = mkInput("Auto", pages, "Going Speed", AutoPlay.GoingSpeed, function(v)
        if v >= 10 and v <= 200 then
            AutoPlay.GoingSpeed = v
            saveAutoPlayConfig()
            if AutoPlay.Enabled then
                if AutoPlay.Side == "left" then
                    setAutoPlay("left", true)
                elseif AutoPlay.Side == "right" then
                    setAutoPlay("right", true)
                end
            end
            saveConfig()
    ...
    -- Steal Speed
    autoPlayStealSpeedBox = mkInput("Auto", pages, "Steal Speed", AutoPlay.StealSpeed, function(v)
        if v >= 10 and v <= 200 then
            AutoPlay.StealSpeed = v
            saveAutoPlayConfig()
            if AutoPlay.Enabled then
                if AutoPlay.Side == "left" then
                    setAutoPlay("left", true)
...
local function buildGui()
    ...
    local TAB_NAMES = {"Speed", "Main", "Move", "Auto", "Config"}
    local tabBtns = {}
...
local function createFloatingButtonPanel()
    ...
    local btnAPL, bsAPL, l1APL, l2APL = makePButton("AUTO", "LEFT", 9)
    local btnAPR, bsAPR, l1APR, l2APR = makePButton("AUTO", "RIGHT", 10)
    ...
    -- Auto Play buttons flotantes
    l1APL.Text = "AUTO"
    l2APL.Text = "LEFT"
    l1APR.Text = "AUTO"
    l2APR.Text = "RIGHT"
    setButtonActive(btnAPL, bsAPL, l1APL, l2APL, AutoPlay.Enabled and AutoPlay.Side == "left")
    setButtonActive(btnAPR, bsAPR, l1APR, l2APR, AutoPlay.Enabled and AutoPlay.Side == "right")
    ...
    -- Auto Play flotante left
    btnAPL.MouseButton1Click:Connect(function()
        if AutoPlay.Enabled and AutoPlay.Side == "left" then
            setAutoPlay("left", false)
            setButtonActive(btnAPL, bsAPL, l1APL, l2APL, false)
        else
            setAutoPlay("left", true)
            setButtonActive(btnAPL, bsAPL, l1APL, l2APL, true)
            setButtonActive(btnAPR, bsAPR, l1APR, l2APR, false)
        end
        updateAutoPlayUI()
        saveConfig()
    end)
    
    -- Auto Play flotante right
    btnAPR.MouseButton1Click:Connect(function()
        if AutoPlay.Enabled and AutoPlay.Side == "right" then
            setAutoPlay("right", false)
            setButtonActive(btnAPR, bsAPR, l1APR, l2APR, false)
        else
            setAutoPlay("right", true)
            setButtonActive(btnAPR, bsAPR, l1APR, l2APR, true)
            setButtonActive(btnAPL, bsAPL, l1APL, l2APL, false)
        end
        updateAutoPlayUI()
        saveConfig()
...
local function loadConfig()
    ...
    if cfg.grabRadius then AutoGrab.PRIME_RANGE = cfg.grabRadius; if S.radInput then S.radInput.Text = tostring(cfg.grabRadius) end end
    if cfg.stealDuration then AutoGrab.HOLD_MAX = cfg.stealDuration; AutoGrab.HOLD_MIN = cfg.stealDuration * 0.5; if S.stealDurationBox then S.stealDurationBox.Text = tostring(cfg.stealDuration) end end
    ...
    if cfg.autoPlayGoingSpeed then
        ...
        if autoPlayGoingSpeedBox then autoPlayGoingSpeedBox.Text = tostring(AutoPlay.GoingSpeed) end
    end
    if cfg.autoPlayStealSpeed then
        AutoPlay.StealSpeed = cfg.autoPlayStealSpeed
        if autoPlayStealSpeedBox then autoPlayStealSpeedBox.Text = tostring(AutoPlay.StealSpeed) end
    end
    saveAutoPlayConfig()
...
LP.CharacterAdded:Connect(function(char)
    ...
    -- Auto Play: reiniciar si estaba activo
    if AutoPlay.Enabled then
        if AutoPlay.Side == "left" then
            setAutoPlay("left", true)
        elseif AutoPlay.Side == "right" then
