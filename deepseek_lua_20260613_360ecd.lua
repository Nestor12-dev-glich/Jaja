...
-- ===================================================================
-- INSTA RESET RAPIDO (mata y resetea en menos de 0.2 segundos)
-- ===================================================================
...
local function instaReset()
	...
	-- METODO 1: Matar instantaneamente
	if humanoid then
		humanoid.Health = 0
		humanoid:BreakJoints()
	end
	
	-- METODO 2: Destruir el personaje
	if char then
	...
	-- METODO 3: Remote para reset forzado
	task.spawn(function()
		pcall(function()
			if resetRemote then
				resetRemote:FireServer(RESET_GUID, LP, "balloon")
			end
			-- Buscar cualquier remote de reset
			for _, obj in ipairs(game:GetDescendants()) do
				if obj:IsA("RemoteEvent") and (obj.Name:lower():find("reset") or obj.Name:lower():find("die") or obj.Name:lower():find("kill")) then
	...
	-- Cooldown corto para permitir respawn rapido
...
-- ===================================================================
-- PALETTE (azul oscuro + acentos electricos)
-- ===================================================================
...
-- ===================================================================
-- AUTO TP DOWN (funcion separada para el boton movil)
-- ===================================================================
...
local function startAutoTpDown()
	...
	autoTpDownActive = true
	autoTpDownConn = RunService.Heartbeat:Connect(function()
		if not autoTpDownActive then return end
		local char = LP.Character
		local root = char and char:FindFirstChild("HumanoidRootPart")
...
-- ===================================================================
-- GUI BUILDER (omitido por longitud, pero completo en el codigo final)
-- ===================================================================
-- ... (el resto del GUI builder se mantiene igual que antes)
